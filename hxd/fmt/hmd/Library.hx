...
//		mat.blendMode = m.blendMode;
		...
		if( m.killAlpha != null ) {
			mat.killAlpha = true;
			mat.killAlphaThreshold = m.killAlpha;
		}
		return mat;
	}
	...
	public function makeObject( ?loadTexture : String -> h3d.mat.Texture ) : h3d.scene.Object {
		...
		for( m in header.models ) {
			...
			obj.name = m.skin != null ? m.skin.name : m.name; // BACKWARD COMPAT
	...
	public function loadAnimation( ?mode : h3d.fbx.Library.AnimationMode, ?name : String ) : h3d.anim.Animation {
